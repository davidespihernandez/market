...
function getMarketFile(filePath, callback){
    models.MarketFile.findOne({filePath: filePath}, function (err, marketFileDoc) {
        ...
        if(marketFileDoc){
            ...
            Measure.remove({marketFile: marketFileDoc._id}, function(err){
            ...
            stream.on('end',function(){
                ...
                getMarketFile(filePath, function(marketFileDoc){
                    ...
                    for (var i = 1, len = lines.length; i < len; i++) {
                        ...
                        var dateFrom = Date.parse(fields[0].substring(0,4) + "-" + fields[0].substring(4,6) + "-" + fields[0].substring(6,8))
                        var dateTo = Date.parse(fields[1].substring(0,4) + "-" + fields[1].substring(4,6) + "-" + fields[1].substring(6,8))
                        //TODO: add hour
...
exports.fileNameInfo = function(fileName){
